...
internal sealed class ScopedFishingTransport(IGameHttpClient reads, string initialFingerprint, string worldFingerprint,
    ...
    public async Task<HttpResponseMessage> PostAsync(string path, HttpContent? content = null)
    {
        ...
        try
        {
            ...
            try
            {
                ...
                if (state is null || state.Name != "gllab" || state.Layer != "overworld" || state.Inventory.GetValueOrDefault("copper_ore") < 2 || state.Inventory.GetValueOrDefault("sunflower") < 40 || state.Inventory.GetValueOrDefault("event_ticket") < 1 ||
                    _observed.Character.GetProperty("fishing_level").GetInt32() != 1 || state.FreeUnits < 1 ||
                    (move ? state.MapId != 379 || Sent != 0 : state.MapId != 385))
                    throw new ActionFailureException(ActionFailureKind.Rejected);
                if (move)
                {
                    ...
                    if (body.RootElement.ValueKind != JsonValueKind.Object || body.RootElement.EnumerateObject().Count() != 1 ||
                        !body.RootElement.TryGetProperty("map_id", out var id) || !id.TryGetInt32(out int map) || map != 385)
                        throw new ActionFailureException(ActionFailureKind.Rejected);
                }
            ...
            try
            {
                ...
                if (after is null || after.Name != "gllab" || after.MapId != 385 || after.Layer != "overworld" ||
                    after.Inventory.GetValueOrDefault("copper_ore") < 2 || after.Inventory.GetValueOrDefault("sunflower") < 40 || after.Inventory.GetValueOrDefault("event_ticket") < 1)
                    throw new ActionFailureException(ActionFailureKind.UnknownOutcome);
                if (character.GetProperty("fishing_level").GetInt32() >= 2) { _latched = true; stop(); }
                return response;
            }
