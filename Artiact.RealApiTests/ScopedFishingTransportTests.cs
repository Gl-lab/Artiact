...
public class ScopedFishingTransportTests
{
    [Theory]
    [Trait("Category", "RealApiOffline")]
    [InlineData(39, 1)]
    [InlineData(40, 0)]
    public async Task ProtectedStocksCannotDisappearAfterMove(int sunflower, int ticket)
    {
        var f = new Fixture(); await f.Observe();
        using (await f.Transport.PostAsync("/my/gllab/action/move", new StringContent("{\"map_id\":385}"))) { }
        f.Map = 385; f.Sunflower = sunflower; f.Ticket = ticket;
        await f.Observe();
    ...
    [InlineData("/my/gllab/action/move", "{\"map_id\":379}")]
    [InlineData("/my/gllab/action/move", "{\"map_id\":385,\"x\":1}")]
    ...
    public async Task RequiresNewObservationAndStopsAtMilestone()
    {
        var f = new Fixture();
        await f.Observe();
        using var move = await f.Transport.PostAsync("/my/gllab/action/move", new StringContent("{\"map_id\":385}"));
        await Assert.ThrowsAsync<ActionFailureException>(() => f.Transport.PostAsync("/my/gllab/action/gathering"));
        f.Map = 385;
        await f.Observe();
        f.ReplyLevel = 2;
    ...
    public async Task RejectsChangedInitialState(int level, int map, int copper)
    {
        ...
        await f.Observe();
        await Assert.ThrowsAsync<ActionFailureException>(() => f.Transport.PostAsync("/my/gllab/action/move", new StringContent("{\"map_id\":385}")));
    ...
    public async Task LostReplyLatchesEvenAfterNewObservation()
    {
        ...
        await f.Observe();
        await Assert.ThrowsAsync<HttpRequestException>(() => f.Transport.PostAsync("/my/gllab/action/move", new StringContent("{\"map_id\":385}")));
        await f.Observe();
        await Assert.ThrowsAsync<ActionFailureException>(() => f.Transport.PostAsync("/my/gllab/action/move", new StringContent("{\"map_id\":385}")));
        Assert.Equal(1, f.Sent);
    }
    ...
    {
        public int Map = 379, Level = 1, Copper = 2, Sunflower = 40, Ticket = 1, ReplyLevel = 1, Sent;
        public bool Stopped, LoseReply, ChangedWorld;
        public Clock Clock { get; } = new();
        public ScopedFishingTransport Transport { get; }
        ...
        { name = "gllab", map_id = map ?? Map, layer = "overworld", inventory_max_items = 100,
            inventory = new[] { new { code = "copper_ore", quantity = Copper }, new { code = "sunflower", quantity = Sunflower }, new { code = "event_ticket", quantity = Ticket } }, fishing_level = level ?? Level });
        public Fixture()
        {
            ...
            Transport = new(this, baseline.Fingerprint, baseline.WorldFingerprint, "fixture", async (path, _) =>
            {
                ...
                return Reply(new { data = new { character = Character(385, ReplyLevel) } });
            }, () => Stopped = true, Clock);
        }
