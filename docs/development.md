...
The solution runs application tests, socket-free MockService tests and `RunProcessRecoveryTests`. The latter launches the real application host as a child process with sentinel credentials and a loopback-only proxy to TestServer, kills it at transport/journal boundaries, and verifies recovery. It requires child-process and loopback-listener support. Run state is isolated in a temporary directory and removed after process termination. The separate `Artiact.RealApiTests` project contains offline configuration/HTTP-boundary checks and an explicit live smoke, but is not part of the solution. Record exact totals with the tested revision in change evidence; do not use a static test count as a completion gate. The default solution performs no production network access. Coverlet is installed as a collector, with no enforced threshold:
