...
R2 adds explicit `Execution:Mode=Bounded`. Configure the same API/Portfolio as Inspect and set `AllowActions=true`, stable `RunId`, absolute `RunDirectory`, positive `MaxActions`, `MaxDecisions` and `MaxSeconds` (1–86400), with MaxNoProgress <= MaxDecisions. R19 preparation permits small positive decision budgets without the earlier arbitrary minimum of ten. Defaults for budgets are 100/200/3600; directory and ID have no defaults. Live origin additionally requires the existing live opt-in and separate rollout evidence. Default host mode remains Inspect.
