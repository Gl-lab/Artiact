...
R19's original two-action/120-second preflight was infeasible. After budget reassessment and explicit scoped approval with R16 open, the [gllab/alchemy2 live trial](../openspec/changes/scoped-operator-live/live-evidence.md) passed:13 verified actions,687.7 elapsed seconds,370 confirmed cooldown seconds. Gathering actually cost30 seconds rather than the configured5-second estimate. This is one route, not general cooldown calibration or continuous-operation evidence. Scope-triggered stop uses Cancelled and the panel's generic "stopped by operator" label; final cooldown was recorded but not fully awaited. The harness exited before the original panel's next poll, so the retained result was verified through a separately restarted read-only panel. Manual Stop during POST and forced process crash remain locally tested rather than repeated in the live game.
...
On 2026-09-08 the user explicitly waived further historical-transient investigation as an R16 completion gate. R16 is accepted with this known limitation. No failure guards, diagnostics or regression tests were removed, and the transient is not marked fixed.
