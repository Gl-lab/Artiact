...
R19's original read-only preflight on 2026-09-08 found no actionable route under two actions/120 seconds. The user subsequently requested a [budget reassessment](../openspec/changes/reassess-live-budget/verification.md): the same fresh-snapshot matrix selects alchemy at 13 actions/134 planner seconds; the proposed execution ceiling is 16 actions/20 decisions/900 seconds to include repeated observations and headroom. This is an estimate from one acquisition timing sample, not live completion evidence. No game actions were sent. R19 real operator acceptance, its R16 prerequisite decision and R20 dependency remain open.
