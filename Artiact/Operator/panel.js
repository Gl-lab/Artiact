...
let controlToken=null,currentRun=null,executor='Idle',connected=false,busy=false,pollTimer=null,polling=false;
async function poll(){
    if(polling)return;
    clearTimeout(pollTimer);polling=true;
    try{const response=await fetch('/operator/snapshot',{cache:'no-store'});if(!response.ok)throw Error();render(await response.json())}
...
function updateControls(){
    ...
    el('recover').disabled=busy||running||!currentRun?.RunId;
    el('archive').disabled=busy||running||!currentRun?.IdentityDigest||!['Completed','Stopped'].includes(currentRun.Status)||currentRun.InterventionRequired;
...
async function command(path,body){
    ...
    const result=await response.json();text('control-result',messages[result.Reason]??reasons[result.Reason]??('Операция отклонена. Код: '+result.Reason));return result;
}
...
async function prepare(start){
    ...
    await guarded(async()=>{
        ...
        await poll();
        if(started.Accepted){executor='Running';updateControls()}
    });
}
