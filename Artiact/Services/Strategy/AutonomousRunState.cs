...
public sealed record AutonomousRunState(string Algorithm, GoalDiscoveryEvidence? Active, ImmutableArray<GoalTransition> History, string? ActiveWorld = null)
{
    ...
    public static bool ValidGoal(GoalDiscoveryEvidence goal) => goal.Skill == "recovery" ? goal.Version == "recovery-v1" &&
        goal.Target is > 0 and <= 1_000_000 && goal.UnlockUtility is > 0 and <= 4 && goal.ProgressUtility == 0 && goal.RecipeUtility == 0 : goal.Version == AutonomousGoalDiscovery.Version &&
    ...
    public AutonomousRunState CompleteObserved(StrategyObservation state) => Active is { } goal &&
        state.Character.TryGetProperty(goal.Skill == "recovery" ? "hp" : goal.Skill + "_level", out var level) && level.TryGetInt32(out int observed) && observed >= goal.Target
