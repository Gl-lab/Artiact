...
- Reset with `POST /__mock/reset` and `{ "scenario": "basic-mining" }`, then load `GET /characters/MockHero` once before action calls. Repeated character reads preserve mutations.
- Keep state process-local unless persistence is an explicit requirement.
- Keep the normative scenario in `BasicMiningScenario.json`; preserve atomic state/trace transitions, deep snapshots and virtual cooldown semantics. Do not introduce production forwarding.
