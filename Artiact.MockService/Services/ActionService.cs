...
public class ActionService : IActionService
{
    ...
    public Character GatheringAction( string? characterName )
    {
        ...
        if ( action == null )
        {
            throw new ApplicationException( $"No Gathering action found position ({character.X}, {character.Y})" );
        }
    ...
    public Character CraftingAction( string? characterName, Item item )
    {
        ...
        if ( action == null )
        {
            throw new ApplicationException(
                $"No crafting action found for item {item.Code} at position ({character.X}, {character.Y})" );
        }

        _logger.LogInformation(
            "Персонаж {CharacterName} начал крафт предмета {ItemCode} в количестве {Quantity} на позиции ({X}, {Y})",
    ...
    private void ApplyChanges( CharacterExtension character,
                               Changes changes,
                               int multiplier = 1 )
    {
        ...
        if ( changes.Inventory != null )
        {
            foreach ( InventoryChange itemChange in changes.Inventory )
            {
                ...
                if ( existingItem != null )
                {
                    ...
                    _logger.LogInformation(
                        "Изменение количества предмета {ItemCode}: {OldQuantity} -> {NewQuantity} ({Difference:+0;-#})",
